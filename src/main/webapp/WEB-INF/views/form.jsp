...
<body>
	...
	<form:form modelAttribute="member2" action="/action" method="post">
		...
		<!-- path="id" 락고 하면 name="id"와 같은 효과 -->
		<form:input path="id" readonly="true" />
		<br>
		
		<label for="name">Name:</label>
		<form:input path="name" />
		<!-- <input type="text" id="id" name="id" value="${member2.id}" readonly -->
		<br>
		...
		<!-- 컨트롤러 : @ModelAttribute("modelVo") modelVo.put("bloodType", bloodType); -->
		<form:select path="bloodType" items="${modelVo.bloodType}" />
		<br>
		
		<label for="gender">Gender:</label>
		<!-- 컨트롤러 : @ModelAttribute ("modelVo") modelVo.put("gender", gender); -->
